    ...
    try {
        ...
        if (filters.page !== undefined && (isNaN(filters.page) || filters.page < 1)) {
            return errorHandler.handleValidationError(
                res,
                'Page must be a positive number starting from 1'
            );
        }
...
const getMyTasksController = async (req: Request, res: Response) => {
    try {
        ...
        if (
            filters.status &&
            !['available', 'in_progress', 'completed', 'cancelled'].includes(filters.status)
        ) {
        ...
        return errorHandler.handleServerError(
            res,
            'Internal server error while retrieving user tasks'
        );
    }
};
...
const getTaskApplicationsController = async (req: Request, res: Response) => {
    try {
        ...
        return errorHandler.handleServerError(
            res,
            'Internal server error while retrieving task with applications'
        );
    }
};
...
const applyToTaskController = async (req: Request, res: Response) => {
    try {
        ...
        return errorHandler.handleServerError(
            res,
            'Internal server error during application creation'
        );
    }
};
...
const selectApplicantController = async (req: Request, res: Response) => {
    try {
        ...
        return errorHandler.handleServerError(
            res,
            'Internal server error during applicant selection'
        );
    }
};
...
const deselectApplicantController = async (req: Request, res: Response) => {
    try {
        ...
        return errorHandler.handleServerError(
            res,
            'Internal server error during applicant deselection'
        );
    }
};
...
const checkApplicationController = async (req: Request, res: Response) => {
    try {
        ...
        return errorHandler.handleServerError(
            res,
            'Internal server error while checking application'
        );
    }
};
...
const withdrawApplicationController = async (req: Request, res: Response) => {
    try {
        ...
        if (response.success) {
            ...
            return errorHandler.handleServerError(
                res,
                'Internal server error during application withdrawal'
            );
        }
    } catch (error) {
...
const cancelTaskController = async (req: Request, res: Response) => {
    try {
        ...
        return errorHandler.handleServerError(
            res,
            'Internal server error during task cancellation'
        );
    }
};
...
const getMyApplicationsController = async (req: Request, res: Response) => {
    try {
        ...
        const filters: GetUserApplicationsFilters = {
            ...
            limit,
            status: status
                ? (status as 'applied' | 'selected' | 'in_progress' | 'completed' | 'cancelled')
                : undefined,
        };
        ...
        return errorHandler.handleServerError(
            res,
            'Internal server error fetching user applications'
        );
    }
};
