        ...
        if (this.newPasswordForm.valid) {
            ...
            this.passwordRecoveryService.attemptResetPassword(NewPassword).subscribe({
                next: (response) => {
                    ...
                    this.authService.notificationMessage.set(
                ...
                error: (error) => {
                    if (error.error.errors) {
                        ...
                        this.authErrorMessage.set(
                            error.status === 400
                                ? 'No se ha validado ningún código, intenta de nuevo'
                                : error.status === 429
                                  ? 'Demasiados intentos de restablecimiento. Por favor, intenta más tarde.'
