        ...
        if (this.signUpForm.valid) {
            ...
            this.userService.attemptSignUp(registerRequest).subscribe({
                next: (response) => {
                    ...
                    this.userService.notificationMessage.set(
                ...
                error: (error) => {
                    this.authErrorMessage.set(
                        error.status === 400
                            ? 'Algo fallo en el formulario, vuelve a intentarlo.'
                            : error.status === 409
                              ? 'Ya fue creado un usuario con este correo electrónico o este numero de identificación.'
                              : error.status === 429
