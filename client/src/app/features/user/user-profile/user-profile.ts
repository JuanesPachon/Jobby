...
export default class UserProfile implements OnInit, OnDestroy {
    ...
    get userNotification() {
        return this.userService.userNotification;
    }
    get notificationMessage() {
        return this.userService.notificationMessage;
    }
    ...
    saveDescription(): void {
        if (this.descriptionControl.valid && !this.isLoadingDescription()) {
            ...
            this.userService.editUserProfile(updateData).subscribe({
                ...
                error: (error) => {
                    this.errorMessageDescription.set(
                        'Error al actualizar la descripción, inténtelo nuevamente más tarde.'
                    );
    ...
    openDeleteModal(
        type: 'experience' | 'skill' | 'document',
        resourceName: string,
        resourceId: number
    ): void {
        const typeNames = {
