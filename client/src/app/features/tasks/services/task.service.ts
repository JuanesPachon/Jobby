...
export class TaskService {
    ...
    applyToTask(taskId: number): Observable<any> {
        return this.http.post<any>(
            `${environment.apiUrl}/tasks/apply/${taskId}`,
            {},
            {
                withCredentials: true,
            }
    ...
    checkApplicationStatus(taskId: number): Observable<any> {
        ...
        return this.http
            .get<any>(`${environment.apiUrl}/tasks/${taskId}/check-application`, {
                withCredentials: true,
            })
            .pipe(
                tap((response) => {
    ...
    getMyPublishedTasks(filters?: {
        limit?: number;
        page?: number;
        status?: string;
    }): Observable<PublishedTasksResponse> {
        const params = new URLSearchParams();
        ...
        const url = queryString
            ? `${environment.apiUrl}/my-tasks?${queryString}`
            : `${environment.apiUrl}/my-tasks`;
    ...
    getPublishedTaskWithApplications(taskId: number): Observable<TaskWithApplicationsResponse> {
        return this.http.get<TaskWithApplicationsResponse>(
            `${environment.apiUrl}/my-tasks/applications/${taskId}`,
            {
                withCredentials: true,
            }
        );
    ...
    selectApplicant(taskId: number, applicantId: number): Observable<any> {
        return this.http.put<any>(
            `${environment.apiUrl}/my-tasks/select/${taskId}`,
            {
    ...
    deselectApplicant(taskId: number): Observable<any> {
        return this.http.put<any>(
            `${environment.apiUrl}/my-tasks/deselect/${taskId}`,
            {},
            {
                withCredentials: true,
            }
    ...
    startTask(taskId: number): Observable<any> {
        return this.http.put<any>(
            `${environment.apiUrl}/my-tasks/start/${taskId}`,
            {},
            {
                withCredentials: true,
            }
    ...
    completeTask(taskId: number): Observable<any> {
        return this.http.put<any>(
            `${environment.apiUrl}/my-tasks/complete/${taskId}`,
            {},
            {
                withCredentials: true,
            }
    ...
    getMyAppliedTasks(filters?: {
        ...
        const queryString = params.toString();
        const url = queryString
            ? `${environment.apiUrl}/my-applications?${queryString}`
            : `${environment.apiUrl}/my-applications`;
