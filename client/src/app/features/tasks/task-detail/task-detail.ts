...
export default class TaskDetail implements OnInit {
    ...
    get returnButtonText(): string {
        return this.returnRoute() === '/dashboard' ? 'Volver al dashboard' : 'Volver a las tareas';
    }
    ...
    get applyButtonClass(): string {
        const baseClass =
            'postular w-full md:w-auto px-4 md:px-20 lg:px-24 py-2 lg:py-2.5 rounded-xl font-semibold shadow-sm border border-black text-center text-sm lg:text-base flex items-center justify-center gap-2 transition-colors';
        ...
        if (this.hasApplied()) {
            return (
                baseClass +
                ' bg-yellow-400 text-black hover:bg-main-blue hover:text-white cursor-pointer'
            );
        }
        if (this.isApplying()) {
            return baseClass + ' bg-gray-400 text-white cursor-not-allowed';
        ...
        return (
            baseClass + ' bg-gradient-to-b bg-main-blue text-white hover:bg-blue-600 cursor-pointer'
        );
    }
            ...
            this.taskService.withdrawApplication(task.id).subscribe({
                next: (response) => {
                    if (response.success) {
                        ...
                        this.taskService.taskNotificationMessage.set(
                            'Te despostulaste correctamente'
                        );
                ...
                error: (error) => {
                    if (
                        error.status === 400 &&
                        error.error?.message?.includes('Application status is not "applied"')
                    ) {
                        this.taskService.taskNotification.set(true);
                        this.taskService.taskNotificationMessage.set(
                            'No puedes despostularte, el creador de la tarea ya te ha seleccionado'
                        );
                        ...
                        this.taskService.taskNotification.set(true);
                        this.taskService.taskNotificationMessage.set(
                            'Error al despostularse. Intenta nuevamente'
                        );
